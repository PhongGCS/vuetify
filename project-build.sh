...
# delete the machine if exists
./project-delete.sh $LOCAL_DEV_HOSTNAME

# prepare IP addresses
source scripts/ip-allocate.sh 1

docker-compose -p $DOCKER_CONTAINER_NAME build --force-rm 
...
docker-compose exec web bash -c "cd /ssl && openssl genrsa -out device.key && openssl req -new -key device.key -out device.csr -subj '/C=VN/ST=VN/L=HCMC/O=Conceptual Studio/OU=Conceptual/CN=$LOCAL_DEV_HOSTNAME' -nodes -config ssl.conf && openssl x509 -req -in device.csr -CA /source/files/docker/rootCA.pem -CAkey /source/files/docker/rootCA.key -CAcreateserial -out device.crt -days 5000 -sha256 -extfile v3.ext"
